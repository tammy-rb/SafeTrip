...
const getById = async (id) => {
  ...
  try {
    const [rows] = await connection.query(
      'SELECT id, first_name, last_name, id_number, class_name FROM Teachers WHERE id = ?',
      [id],
    );
    return rows[0] || null;
  } finally {
...
const getAll = async (filters = {}) => {
  const connection = await connectDB();
  try {
    let sql = 'SELECT id, first_name, last_name, id_number, class_name FROM Teachers';
    const conditions = [];
    const params = [];
...
const insert = async (teacher) => {
  ...
  try {
    const [result] = await connection.query(
      ...
      [teacher.first_name, teacher.last_name, teacher.id_number, teacher.class_name],
    );
